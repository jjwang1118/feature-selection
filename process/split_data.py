...
def split_data(save_path, split_ratio=0.8):
    ...
    train_set.to_csv(f"{save_path}/train.csv", index=False)
    test_set.to_csv(f"{save_path}/test.csv", index=False)
    ...
    with open(f"{save_path}/split_info.json", "w", encoding="utf-8") as f:
        json.dump(split_info, f, indent=4, ensure_ascii=False)
    
    print(f"✅ Train set ({len(train_data)} samples) saved to {save_path}/train.csv")
    print(f"✅ Test set ({len(test_data)} samples) saved to {save_path}/test.csv")
    print(f"✅ Split info saved to {save_path}/split_info.json")
